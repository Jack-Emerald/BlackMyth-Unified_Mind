...
class EldenEnv(gym.Env):
    ...
    def check_for_conclusion_screen(self, frame):
        # The way we determine if we are in a loading screen is by checking if the text "return" or "vanquished" is in the screen. If it is we are in a conclusion screen. If it is not we are not in a loading screen.
        vanquish_text_image = frame[150:150+50, 310:310+250]  # Cutting the frame to the location of the text "next" (bottom left corner)
        vanquish_text_image = cv2.resize(vanquish_text_image, ((205 - 155) * 3, (1040 - 1015) * 3))
        lower = np.array([0, 0, 75])  # Removing color from the image
        upper = np.array([255, 255, 255])
        hsv = cv2.cvtColor(vanquish_text_image, cv2.COLOR_RGB2HSV)
        mask = cv2.inRange(hsv, lower, upper)
        pytesseract_output = pytesseract.image_to_string(mask, lang='eng',
                                                         config='--psm 6 --oem 3')  # reading text from the image cut out
        in_loading_screen = "Vanquished" in pytesseract_output or "vanquished" in pytesseract_output  # Boolean if we see "next" in the text
